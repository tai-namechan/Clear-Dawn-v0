...
class StartFoodMenuEstimateService
{
     ...
     */
    public function start(User $user, string $storeName, string $menuName): FoodLookupRequest
    {
        $this->ai->assertWithinQuota($user->id);

        return DB::transaction(function () use ($user, $storeName, $menuName): FoodLookupRequest {
