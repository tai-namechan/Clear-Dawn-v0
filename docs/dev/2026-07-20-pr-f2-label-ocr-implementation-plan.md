...
- 商品DBの外部購入
- 料理写真からの栄養推定
- **バーコードなしの成分表単独登録**（F1 miss 経由のみ。将来拡張として温存 → 未決事項Q2）
...
**採用: F1 の `BarcodeLookupModal` 内に統合**（別モーダルにしない）。

- `not_found` / `failed` 到達時（`lookupId` が手元にある状態）に「成分表を撮影して登録」CTA を表示 → 同一モーダル内の新ステップ `ocr_capture` へ。
- 理由: フロー全体（scan → 照合 → miss → 撮影 → confirm）が1つの連続体験であり、polling・confirm・エラーメッセージ枠を全て共有できる。別モーダルだと lookupId の受け渡しと状態リセットが複雑化する。
- **バーコードなしは許さない**。upload API は既存 lookup（= barcode 確定済み）への添付のみ。`food_lookup_requests.barcode` の NOT NULL 制約を維持でき、confirm の barcode 紐づけも無変更で成立する。
...
- 遷移: `not_found | failed` → *(upload)* → `ocr_pending` → *(Job)* → `found`（source=`label_ocr`）| `failed`（error_code で区別）。
- 遷移は**条件付きUPDATE**（`whereIn('status', ['not_found','failed'])->update(...)`）で行い、二重アップロード競合を排除（Kioku claim パターン）。
...
- barcode 紐づけ: **confirm 時**（既存 `ConfirmFoodLookupService` が `$lookup->barcode` を使う実装のまま。upload 時に food_items へ触らない）。
- migration: **enum は string カラムなので DB migration 不要**。PHP enum へのケース追加のみ。
...
```
POST /meals/barcode-lookup/{lookupId}/label-image   (auth + verified, throttle:10,1)
```

- FormRequest `StoreFoodLabelImageRequest`:
...
- Controller（`FoodBarcodeLookupController@storeLabelImage` に追加。新Controllerは作らない）:
  1. `user_id` スコープ + `whereKey($lookupId)` + `whereIn('status', [NotFound, Failed])` → 見つからなければ 404
  2. ファイルを private disk へ store（Kioku パターン: **store → DB → 失敗時ファイル削除**）
  3. 条件付きUPDATE で `status=ocr_pending, temp_image_path=..., source=null, result=null, error_code=null` に遷移。0行なら（並行アップロードに負けた）保存済みファイルを削除して 409
  4. `DB::afterCommit` で `LookupFoodLabelOcrJob::dispatch($lookup->id)`
  5. 202 `{"status":"ocr_pending"}`
- **quota 事前チェック**: controller で `AiGateway::assertWithinQuota($userId)` を呼び、超過なら 422 `{"message":"今月のAI利用枠を使い切りました。来月また利用できます。"}`（画像を保存する前に弾く）。Job 側の reserve が最終権威（レース時は Job が `ocr_quota_exceeded` で失敗させる）。

### E. OCR Job

- クラス: `App\Jobs\LookupFoodLabelOcrJob`（default Queue。personal 規模で dedicated 不要 — §13.4 の選択肢から default を採用）
- **本番 Queue 前提の確認（→ Q4）**: Laravel Cloud の Managed queue は「integrations」の名前で表示されている。コード上 `onQueue('integrations')` は Google Calendar 系 2 Job のみで、kioku 文字起こし・F1 OFF 照合・本 Job は **default キュー**に載る。この worker が default キューも処理する設定（例: `default,integrations`）であることを要確認。kioku 文字起こしが本番で動いた実績があれば default も処理されている。
...
- `BarcodeLookupModal.vue` 拡張:
  - Step 追加: `ocr_capture`（撮影/選択 + プレビュー + 送信）
...
| 1 | Enum ケース追加 + `config/meals.php` + Factory state | `FoodLookupStatus::OcrPending`, factory `ocrPending()` |
| 2 | Upload API（FormRequest / controller action / route / quota 事前チェック） + Feature テスト | `StoreFoodLabelImageRequest`, route, テスト |
| 3 | AiGateway PHPDoc 緩和 + `LookupFoodLabelOcrJob`（プロンプト / schema 検証 / 失敗分類 / 画像削除） + Job テスト | Job + テスト |
| 4 | confirm 経路の確認（`label_ocr` source の Found を confirm できる Feature テスト追加。実装変更は原則なし） | テスト |
...
新規:
- `config/meals.php`
- `config/filesystems.php` の `food-label-ocr` スタブ disk（変更扱い）
...
- `app/Http/Controllers/FoodBarcodeLookupController.php`（storeLabelImage 追加）
...
### POST /meals/barcode-lookup/{lookupId}/label-image

Request: `multipart/form-data` — `image`: JPEG/PNG/WebP ≦5MB, 200–8000px

| 条件 | レスポンス |
|---|---|
| 成功 | `202 {"status":"ocr_pending"}` |
| 他ユーザー / 不存在 | 404 |
...
| R4 | Managed queue が default キューを処理しない設定だと F1/F2 の Job が本番で動かない | **F1 にも影響**（OFF 照合 Job も default） | Q4 で確認。処理しない場合は F1 含め `onQueue` 明示 or worker 設定変更 |
| R5 | worker ゼロスケールによる Job 開始遅延（数十秒） | OCR/照合の体感待ち時間増 | ポーリング継続 + 文言で案内。仕様変更なし |

### オーナー判断が必要な質問（最大3）

- **Q1**: PR #143（F1）を先にマージしてよいか？ F2 は F1 のコードに全面依存するため、マージ後に本ブランチを main へ rebase するのが最もクリーン（それまで F1 ブランチ上にスタックして進める）。
- **Q2**: バーコードが読めない商品（成分表だけ撮って登録）は本PRの範囲外（F1 miss 経由のみ）でよいか？ 推奨: 範囲外。
- **Q3**: 「写真は解析後に破棄」の解釈は「終端状態（found/failed 確定）で即削除 + 期限切れ prune が安全網」でよいか？（リトライ中はファイル保持が必要なため）推奨: この解釈。
- **Q4**: Laravel Cloud の Managed queue「integrations」は **default キューも処理する設定**か？（Laravel Cloud のキュー設定画面で処理対象キュー名を確認。kioku 文字起こしが本番で動いていれば Yes）No の場合、F1 の OFF 照合 Job も本番で動かないため、F2 と合わせて対応方針を決める。
