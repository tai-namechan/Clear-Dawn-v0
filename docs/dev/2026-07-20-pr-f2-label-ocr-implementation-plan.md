...
- disk: `config('meals.label_ocr.disk', 'local')`（= `storage/app/private`）。新規 config `config/meals.php` を作る（kioku.php の audio 節と同型。Laravel Cloud では env `MEALS_LABEL_OCR_DISK` で Object Storage 名に差し替え可能）。
