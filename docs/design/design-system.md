...
#### OS共通テキストトークンの使い分け（確定 2026-07-11）

ヨユウ・キオク等で使う `--os-*` テキスト色は3段階。**読ませる・操作させるテキストに `--os-faint` を使わない**（washi背景に対し約2.4:1でWCAG AA未満）。

| トークン | 値 | 許可される用途 |
|---|---|---|
| `--os-ink` | `#2b2924` | 本文・見出し・ボタンラベル |
| `--os-sub` | `#6e6a5e` | 補助テキストの下限（メタ情報・注記・空状態メッセージ・操作アイコン）。これ未満の濃さで実コンテンツを出さない |
| `--os-faint` | `#a39e8f` | **装飾・プレイスホルダー限定**（input placeholder、未チェックの丸枠・飾り罫など、読めなくても操作に支障がないもの） |
