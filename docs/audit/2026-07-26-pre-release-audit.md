...
特に重大なのは **C-1（メール検証が全ルートで無効）** である。誰でも捨てアドレスでアカウントを作り、そのまま全機能にアクセスできる。

> **【訂正】** 当初はここに **C-2（公開登録の停止フラグが機能していない）** を並べ、「2つが重なると非公開のはずのアプリに即時フルアクセスできる」と記載していた。しかし**「非公開のはず」という前提は監査の推測であり、`docs/` に根拠が無かった**。C-2 は仕様どおりとして取り下げ済み（該当節を参照）。C-1 単体の深刻度は変わらない。
...
登録導線を UI から隠していても `/register` は生きているため（これは仕様。C-2 の節を参照）、
「捨てアドレスで登録 → 即フルアクセス」が成立する。**逆に言えば、C-1 を直せば
URL 直打ちで登録されてもメール確認を通すまでアプリの中身には到達できなくなる。**
...
### ~~C-2. 公開登録の停止フラグが飾りになっている~~ → **取り下げ（仕様どおり）**

> **本指摘は Critical から取り下げる。**
>
> 監査時点でこの挙動を「非公開運用を意図した設定が何も止めていない」と評価したが、
> **その前提は誤りだった。** `docs/` にはそもそも登録ポリシーの記述が存在せず、
> 「招待制」「クローズドβ」という前提は `config/app.php` の
> `env('APP_PUBLIC_SIGNUP_ENABLED', false)` という既定値から監査が逆算した推測にすぎない。
> `CLAUDE.md` が「プロダクト仕様の正は `docs/`」と定めている以上、
> 監査は仕様を断定するのではなく **未定義であることを報告すべきだった。**
>
> 実際のポリシーを確認した結果、**「UI に導線を出さないところまでが要件であり、
> URL 直打ちでの登録は現時点では許容する」**ことが確定した
> （`docs/product/signup-policy.md`）。したがって以下に記した「現象」は
> 不具合ではなく仕様であり、修正は不要である。
>
> **監査が本来報告すべきだった内容**: 登録ポリシーが `docs/` に未定義であり、
> フラグの意図が実装からしか読み取れない。まず仕様を確定させること。
>
> なお**未検証ユーザーは C-1 の修正により全ルートから弾かれる**ため、
> URL 直打ちで登録できてもメール確認を通すまでアプリの中身には到達できない。
> 結果として C-1 が、この指摘が求めていた実質的な防御を代替している。
>
> **派生指摘 C-2b は取り下げない。** あれは登録ポリシーとは独立した実バグである（後述）。
>
> 以下は取り下げ前の記述をそのまま残す（経緯の記録として）。
...
**フラグは UI の導線を出し分けるためのものであり、ルートは常に存在する。**
これは `docs/product/signup-policy.md` で確定した仕様である。

さらに実装上の制約として、**ルートをフラグで出し分けてはならない。**
`config/fortify.php` から `Features::registration()` を外すと Wayfinder が
`resources/js/routes/register` を生成しなくなり、それを静的 import している
`AuthRegisterForm.vue` / `AuthLoginForm.vue` が解決できずフロントのビルドごと失敗する。
実際に一度その実装を行い、CI のビルドジョブが落ちて発覚した。
**ビルド生成物の形が実行時フラグで変わる構造は持ち込まない。**

将来ルートを実際に塞ぐ判断をした場合も、`Features::registration()` は残したまま
ミドルウェアで拒否すること。

#### 検証方法

`tests/Feature/Auth/SignupPolicyTest.php` が現在のポリシーを固定している。

- フラグに関わらず `Features::registration()` が有効であること（ビルド保護）
- フラグ false でランディング／ログイン画面の `canRegister` が false になること
- フラグ false でも `/register` に到達できること（受容しているリスクの明示）
- フラグ false でも `canResetPassword` が true であること（C-2b の回帰テスト）
...
「新規ユーザーの導線を出すか」とは何の関係もない。
`public_signup_enabled=false`（**現在の既定であり、実際の運用値**）にした瞬間、
**既存ユーザーがパスワードを忘れると自力で復旧できなくなる**。

ルート自体は生きているため URL を直接知っていれば到達できるが、
UI からの導線が消えるため実質的に使えない。

**これは C-2 と違い、現在の運用値でそのまま踏んでいる実バグである。** 取り下げない。

**あるべき姿**: 2つの判定を分離する。登録導線の表示は `public_signup_enabled` で制御し、
パスワード再設定は常に有効。

**修正で得られるもの**: 登録導線を隠した状態でも既存ユーザーがパスワードを自力で復旧できる。
...
| `APP_PUBLIC_SIGNUP_ENABLED` | 登録導線の出し分けフラグの存在自体が知られない |
...
| 項目 | 評価 | 根拠 |
| --- | --- | --- |
| A01 アクセス制御の不備 | **不合格** | C-1。ただしリソース単位の IDOR は検出されず（下記）。C-2 は仕様どおりとして取り下げ |
...
| A07 認証の失敗 | 要改善 | レートリミッタ自体は良好（`FortifyServiceProvider.php:86-114`）。C-1 が問題 |
