...
| 画面 | ルート | 役割 |
|---|---|---|
| ルーティン（実行ハブ・サイドバー着地） | GET /routines | **今日**（当日プランの開始/再開）と **メニュー**（テンプレ一覧・編集） |
...
ハブタブ（プログラム / ルーティン / 今日/作戦 / 履歴）は維持。サイドバー「ルーティン」の着地は `/routines` のまま変更しない。

### GET /routines の構成（実行ファースト）

1. **今日タブ（既定）** — 当日の実行プラン一覧。開始・再開が第一アクション。プランが無ければ空状態（メニューへ誘導）
2. **メニュータブ** — 再利用テンプレ一覧。「メニューを作る」「今日に追加」「編集」

- 当日プランは既存 `GetTodayQuery` で取得（読み取りのみ。プログラム生成の副作用は `/today` 側）
