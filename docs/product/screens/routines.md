...
| 画面 | ルート | 役割 |
|---|---|---|
| ルーティン（実行ハブ・サイドバー着地） | GET /routines | ページ内タブ **今日 / メニュー / 履歴**。今日のセッション開始が第一ビュー |
...
ハブタブ（プログラム / ルーティン / 今日/作戦 / 履歴）は他画面では維持しうるが、**`/routines` の第一構成は今日 / メニュー / 履歴**。サイドバー「ルーティン」の着地は `/routines` のまま変更しない。

### GET /routines の構成（実行ファースト）

ページ内タブは **今日 / メニュー / 履歴**（この3つが第一構成）。旧ハブタブ（プログラム / ルーティン / 今日/作戦 / 履歴）は `/routines` 上では出さない。プログラム画面自体はサイドバー等から従来どおり利用できる。

1. **今日タブ（既定・第一ビュー）** — 当日のメインセッションを大きく表示し、「セッションを開始」を第一アクションにする。チェックイン・作戦は二次。プランが無ければ空状態（メニューへ誘導）
2. **メニュータブ** — 再利用テンプレ一覧（カテゴリアイコン + 編集 / 今日に追加）
...
1. **主導線は今日の実行。** サイドバー着地の `/routines` は当日プランの開始・再開を第一に出す。テンプレ編集は「メニュー」タブ。実施項目の独立画面は整理用で、ハブタブには出さない。
