...
レイヤー方針は [ADR-0005](../../adr/0005-query-service-eloquent-layering.md) に従う。
Repository は導入しない。読み取りは Query、更新は Service とする。

| 分類 | 作成候補 | 方針 |
|---|---|---|
| Models | LifeArea, MatrixRow, MatrixCell, MatrixCellItem, ActivityLog | ドメイン PK は ULID。user_id FK は bigint |
| Controllers | DashboardController, MatrixCellItemController, LifeAreaController | Controller は薄くする（ADR-0005） |
...
| Queries | GetMatrixBoardQuery | 表示用データ取得を集約（読み取り専用） |
| Services | AddMatrixCellItemService, ToggleMatrixCellItemCompletionService 等 | 更新 + activity_logs 記録（matrix_item_completed / matrix_item_reopened） |
| Enums | MatrixRowKey, ActivityLogEventType 等 | 状態・キーの文字列を安全に扱う |
