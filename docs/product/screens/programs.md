...
実行画面の「ポイント」カードに ` / ` で分割して1行ずつ表示される（[routines.md](./routines.md)）。

**1つのステップに週次変数が2つ以上乗る場合**（例: 木曜の開放弦 = 今週のCUE + 音・身体、
日曜の音階テスト = 音階 + 週末テスト）は、`intent` を使わず `note` の中で
`ラベル：内容` を ` / ` 区切りにする。ラベルだけをまとめて連結すると、
表示時に2件目以降がラベルなしの行になり、1件目のラベルが両方の内容に付いて見えてしまう。
