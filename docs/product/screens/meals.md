...
2. **残り摂取ヒーロー**
   ...
   - 残り PFC（P/F/C）
   - 「次に摂るとよいもの」ヒント
3. **クイック操作** — いつもの食事（未実装・食品検索を開くフォールバック） / 写真（準備中） / 食品検索 / 昨日からコピー（`POST /meals/copy-previous-day`）
...
| 種類 | 保存 | 今日の一覧表示 | 他ユーザー |
|---|---|---|---|
| 料理・商品の外観（`food-photo-estimate/` アップロード） | `meal_entries.photo_path` に 1 枚 | する | 見えない（エントリは user スコープ） |
| 成分表 OCR | 解析後に破棄 | しない | — |
| バーコード照合 | 画像なし | しない | 栄養値だけ自分の `food_items` に残る |

料理写真は、栄養値の出典が `ai_photo_estimate` でもチェーン公式の `nutrition_db` でも保存する。判定はアップロード元のパスであり、照合ソースではない。
確定済みの `meal_entries.photo_path` は、あとから `food_items.source` が変わっても消さない。

- 写真は **その人のその食事記録** にだけ置く。`food_items` やバーコードカタログには載せない。
- 1 エントリ 1 枚まで。複数枚は保存しない（ストレージと転送の負荷を抑える）。
- 他ユーザーが同じバーコードを読んでも、自分の撮影写真は出さない。

## 昨日からコピー

- `POST /meals/copy-previous-day` は対象日に既存エントリがある場合 **拒否**（`copied: 0` + `reason: target_not_empty`）。マージ・置換はしない。
- 前日が空の場合は `copied: 0` + `reason: source_empty`。

## 「いつもの食事」

- 現状は独立機能ではなく、食品検索モーダルを開くフォールバック（未実装プレースホルダ）。専用マスタ・頻度ランキングは未実装。

## `/records` からの導線

パフォーマンス管理ハブ（`/records`）の「食事記録」カードから `/meals` へ誘導する。
ハブ上部サマリに選択日の合計 kcal / PFC を出し、カード内は直近7日のミニ推移（kcal / PFC）を表示する。
当日の区分別内訳や入力 UI は `/meals` 側に寄せ、ハブでは再掲しない。
