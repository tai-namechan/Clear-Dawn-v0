...
- `raw_content` は作成後に変更しない。Model の updating ガードで拒否する
  ...
  - データ修復が必要な場合のみ `Memory::allowRawContentMutation()` を明示して行う
...
- 最大録音時間: **3分**（超過前に自動停止して安全に保存）
- 最大ファイルサイズ: **20MB** 初期値（`config/kioku.php` で変更可能）
...
- サーバー側で MIME・サイズ・duration・所有者を検証する
